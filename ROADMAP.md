...
- **NFC.** Considered, built as far as reading a tag, and withdrawn (#152,
  reverted in #159's revert). What NFC was wanted for is holding two Android
  phones together, and no web page can do that: phone-to-phone needs card
  emulation or a P2P mode, Android removed the P2P mode with Beam, card
  emulation exists only for native apps, and the Web NFC group closed the P2P
  request rather than implementing it. A browser reads and writes passive tags,
  which serves a counter with a sticker - a case for an app that runs a counter,
  not for this demo. The gesture itself is already covered: the QR scan *is*
  this project's hold-the-phones-together, the acoustic channel covers devices
  side by side with no usable camera, and the share sheet covers nearby link
  handover.
